...
if ! pjsip_cli_ok "$AOR_OUT" || ! printf '%s' "$AOR_OUT" | grep -q '1001-aor' \
  || ! pjsip_cli_ok "$EP_OUT" || ! printf '%s' "$EP_OUT" | grep -q '1001'; then
  echo "[deploy] FAIL: pjsip endpoint 1001 / AOR 1001-aor not loaded"
  printf '%s\n' "$AOR_OUT"
  printf '%s\n' "$EP_OUT"
  exit 1
fi
...
echo "[deploy] OK: pjsip endpoint 1001 + AOR 1001-aor loaded, chan_sip unloaded, realm=${SIP_REALM}"
